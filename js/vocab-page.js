...
function render() {
  ...
  main.innerHTML = rows.map((w) => `<div class="card">
    <div class="card-title"><span lang="es">${w.word}</span> <button class="say" title="發音" aria-label="發音">🔊</button> <i>${w.pos || ""}</i> — ${w.zh || ""}</div>
    <div class="card-meta"><span lang="es">${w.example || ""}</span>${w.example_zh ? "<br>" + w.example_zh : ""}${w.level ? " · " + w.level : ""}</div>
  </div>`).join("");
